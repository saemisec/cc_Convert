...
def convert_id(parent_table:str) -> list:
    ...
    SessionLocal = sessionmaker(bind=engine)
    session = SessionLocal()
    return get_id_map(session, parent_table, old_id_field_name="old_id", new_id_field_name="id")

def convert_sec_phase()->list:
    engine = create_engine(DATABASE_URL)
    SessionLocal = sessionmaker(bind=engine)
    session = SessionLocal()
    return get_id_map(session, Second_Phase, old_id_field_name="name", new_id_field_name="id")

def convert_prj_phase()->list:
    engine = create_engine(DATABASE_URL)
    SessionLocal = sessionmaker(bind=engine)
    session = SessionLocal()
    return get_id_map(session, Main_project, old_id_field_name="Abbreviation", new_id_field_name="id")
